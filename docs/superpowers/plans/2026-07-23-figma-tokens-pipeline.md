...
**Порядка недостаточно: нужны составные селекторы.** Обнаружено на кнопке в задаче 11. Селекторы состояний в этой дизайн-системе несут класс — `.ds-interactive:hover`, специфичность `(0,2,0)`, — а контейнерные переключатели той же коллекции только атрибут: `[data-message="error"]`, `(0,1,0)`. На элементе, который и интерактивен, и несёт переключатель, переизлучение побеждает по специфичности, и порядок уже не решает: кнопка опасного действия под наведением становилась синей, тон info вместо error. Симметрично ломался `[data-on-accent]`: подпись недоступной кнопки получала цвет заливки и исчезала.

Поэтому под каждым триггером переизлучения собственный неосновной режим объявляется ещё раз составным селектором — `.ds-interactive:hover[data-message="error"]`, `(0,3,0)`. Селекторы склеиваются попарно и без пробела: оба относятся к одному элементу.

Цена — снова повторяющийся CSS: `state.css` вырос с 800 до 1847 строк, 96 КБ, которые сжимаются до 3 КБ. Все пять слоёв вместе — около 10 КБ в gzip.
